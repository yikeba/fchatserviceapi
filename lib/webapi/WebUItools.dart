...
import 'package:fchatapi/webapi/PayReturnObj.dart';
  ...
  static openWebpay(BuildContext context,Widget? order,PayHtmlObj? pobjhtml,{islive=false}) async {
    ...
    if(FChatApiSdk.isFchatBrower && pobjhtml!=null){
      ...
      fchatpay.pay((value){
        ...
        String url = "${pobjhtml.probj!.returnurl}&payid=$payid";
        PhoneUtil.applog("app支付回调通知，本地进行验证$payid");
        PhoneUtil.applog("网页服务号验证支付url$url");
        html.window.location.href = url;
        Sendorder(payid,pobjhtml.money,pobjhtml.paystr,url).send((value){   //发送消息到预定客户
      ...
      return Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) {
            return Webpaypage(cardobj: null, order: order, pobj: pobjhtml,isLive: islive);
          },
        ),
      );
    }
